...
const Column = ({ applications, column }) => {
  ...
  const applicationCount = applications.filter(app => app.status === column.id).length;

  return (
    <div data-testid="column-container" className="flex h-full w-80 flex-shrink-0 flex-col"> 
      <div className="mb-3 flex items-center justify-between">
        <h2 className="text-lg font-semibold text-gray-800 dark:text-gray-200">{column.title}</h2>
        <span className="rounded-full bg-gray-200 dark:bg-gray-700 px-2.5 py-0.5 text-xs font-medium text-gray-700 dark:text-gray-300">
          {applicationCount}
        </span>
      </div>
      ...
      <div
        ref={ref}
        data-testid="column"
        className={`h-[calc(100vh-10rem)] w-full flex-grow overflow-y-auto rounded-lg 
      ...
      >
        {applications.filter((application) => application.status === column.id).length === 0 ? (
